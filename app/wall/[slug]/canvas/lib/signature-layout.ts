...
export interface SignaturePosition<T> {
  id: string;
  x: number;
  y: number;
  signature: T;
...
export interface SignatureLayout<T> {
  positions: SignaturePosition<T>[];
  revealOrder: string[];
}
...
const computeTargetRadius = (count: number) => {
...
export const computeSignatureLayout = <T,>(
  signatures: Array<{ id: string; signature: T }>,
): SignatureLayout<T> => {
  ...
  const positions = signatures.map((item) => {
    ...
    for (let radius = 0; radius <= targetRadius; radius += RADIAL_STEP) {
      ...
      const circumference = Math.max(
        1,
        Math.PI * 2 * Math.max(ringRadius, RADIAL_STEP),
      );
